...
export default function MyPortfolio() {
  ...
  return (
    <section className="section-container" id="MyPortfolio">
      {/* Header row */}
      <div ref={headerRef} className="scroll-reveal flex flex-col md:flex-row items-start md:items-end justify-between mb-16 gap-6">
        <div>
        ...
        <a
          ...
          className="inline-flex items-center gap-3 px-6 py-3 bg-[#24292f] hover:bg-[#1b1f24] text-white rounded-lg font-semibold transition-colors shadow-lg"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {data?.portfolio?.map((item, index) => {
          const isLive = LIVE_IDS.includes(item.id);
          
          return (
            <div
              key={item.id}
              ref={addCardRef(index)}
              className={`group glass-card overflow-hidden scroll-reveal reveal-delay-${(index % 3) + 1}`}
            >
              <div className="relative aspect-video overflow-hidden">
                ...
                {isLive && <span className="absolute top-4 left-4 bg-emerald-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg z-10">LIVE</span>}
                
                <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 z-20">
                  ...
                  <p className="text-sm text-zinc-300 mb-4 translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-75">{item.description}</p>
                  <a 
                    href={item.link} 
                    target="_blank" 
                    rel="noreferrer" 
                    className="inline-block text-primary font-semibold hover:text-white transition-colors translate-y-4 group-hover:translate-y-0 duration-300 delay-100"
                  >
                    {item.linkLabel} &rarr;
                  </a>
                </div>
